// TCDev.de 2021/08/30
// TCDev.Utilities.BankHolidayBase.cs
...
public static class DateUtilities
{
   ...
   private static readonly Dictionary<CountryCode, Lazy<IWeekendProvider>> _nonUniversalWeekendProviders =
      new()
      {
         ...
         {
            CountryCode.AE, new Lazy<IWeekendProvider>(() => WeekendProvider.SemiUniversal)
         }, // since 2006 // TODO handle launch dates in weekends
         {
   ...
   {
      if (_publicHolidaysProviders.TryGetValue(countryCode, out var provider)) return provider.Value;
   ...
   {
      if (_nonUniversalWeekendProviders.TryGetValue(countryCode, out var provider)) return provider.Value;
   ...
   {
      if (!Enum.TryParse(countryCode, true, out CountryCode parsedCountryCode) || !Enum.IsDefined(typeof(CountryCode), parsedCountryCode)) throw new ArgumentException($"Country code {countryCode} is not valid according to ISO 3166-1 ALPHA-2");
   ...
   {
      if (!Enum.TryParse(countryCode, true, out CountryCode parsedCountryCode)) return null;
   ...
   {
      if (startDate > endDate) throw new ArgumentException("startDate is before endDate");
      ...
      while (currentYear <= endYear)
      {
         ...
         {
            if (item.Date.Date >= startDate.Date && item.Date.Date <= endDate.Date) yield return item;
         }
   ...
   public static IEnumerable<PublicHoliday> GetPublicHolidays(DateTime startDate, DateTime endDate)
   {
      ...
      foreach (var publicHolidayProvider in _publicHolidaysProviders.Keys) items.AddRange(GetPublicHolidays(startDate, endDate, publicHolidayProvider));
   ...
   public static bool IsPublicHoliday(DateTime date, CountryCode countryCode, out PublicHoliday[] publicHolidays)
   {
      ...
      publicHolidays = items.Where(GetPublicHolidayFilter(date))
         .ToArray();
   ...
   {
      var provider = GetPublicHolidayProvider(countryCode);
      if (provider is ICountyProvider countryProvider
          && !countryProvider.GetCounties()
             .ContainsKey(countyCode)) throw new ArgumentException($"Invalid countyCode {countyCode}");
   ...
   /// <param name="countyCode">Federal state</param>
   /// <returns></returns>
   [Obsolete("Use IsPublicHoliday instead", true)]
   ...
   public static DateTime FindLastDay(int year, int month, DayOfWeek day)
   {
      ...
      if (resultedDay == DateTime.MinValue) resultedDay = FindDay(year, month, day, 4);
   ...
   public static DateTime FindDay(DateTime date, DayOfWeek dayOfWeek)
   {
      ...
      if ((int)dayOfWeek >= (int)date.DayOfWeek) return date.AddDays(daysNeeded);
   ...
   public static DateTime FindDayBetween(int yearStart, int monthStart, int dayStart, int yearEnd, int monthEnd, int dayEnd, DayOfWeek dayOfWeek)
   {
      ...
      for (var i = 0; i <= days; i++)
      {
         ...
         if (specificDayDate.DayOfWeek == dayOfWeek) return specificDayDate;
      }
   ...
   {
      if (occurrence == 0 || occurrence > 5) throw new ArgumentException("Occurance is invalid", nameof(occurrence));
      ...
      if (resultedDay > DateTime.DaysInMonth(year, month)) return DateTime.MinValue;
